...
# ATM 1:1 mapping to DB schema.
# List explicitly so we could reuse in grammar and error messages etc
# without duplication
known_fields = ["url", "ds_id", "head", "head_describe", "branches", "tags", "metadata"]
...
?field_select: (field | metadata_field "[" metadata_extractors "]")
...
class SearchQueryTransformer(Transformer):
    ...
    def field_matched(self, args):
        ...
        if op == "=":
            op_func = lambda field, value: field == value
        elif op == "?":
            op_func = lambda field, value: field.ilike(f"%{value}%")
        # Add more operations as necessary

        # Mapping from field names to RepoUrl attributes
        field_map = {
            "url": RepoUrl.url,
            "ds_id": RepoUrl.ds_id,
        ...
        if field in field_map:
            return op_func(field_map[field], value)
        else:
            # Handle unknown fields, possibly raise an error or ignore
            pass

    def search_word(self, arg: Token):
        # Handle generic search words
        import pdb; pdb.set_trace()
        word = args[0]
        return or_(
            RepoUrl.url.ilike(f"%{word}%"),
            RepoUrl.ds_id.ilike(f"%{word}%"),
            # Add more fields as needed
        )

    def search_string(self, args):
        # Similar to search_word, but for quoted strings
        return self.search_word(args)

...
def test_token_get(client, ds_id):
    url = "doesnt.matter"
...
def test_token_bad_url_get(client, ds_id, bad_url):
    response = client.get(f"/v1/datasets/{ds_id}/urls/{bad_url}/token")
...
def test_urls_post_unknown_token(client, ds_id):
    response = client.post(f"/v1/datasets/{ds_id}/urls",
                           json={"token": "unknown",
                                 "url": "doesnt.matter"})
    assert response.status_code == 400
...
def test_register_url(client, ds_id, tmp_path):
    ...
    d_token2 = client.get(
        f"/v1/datasets/{ds_id}/urls/{url_encoded}/token").get_json()
    r_post2 = client.post(f"/v1/datasets/{ds_id}/urls", json=d_token2)
    assert r_post2.status_code == 202
    assert get_status() == "known"


def test_register_url_expired_token(client, ds_id, tmp_path):
    dset = tmp_path / "ds"
    dset.mkdir()
...
def test_register_url_failed_verification(client, ds_id, tmp_path):
    ...
    r_post = client.post(f"/v1/datasets/{ds_id}/urls", json=d_token)
    assert r_post.status_code == 202
    data = client.get(f"/v1/datasets/{ds_id}/urls/{url_encoded}").get_json()

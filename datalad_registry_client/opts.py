...
from datalad.distribution.dataset import Dataset, EnsureDataset, require_dataset
...
def process_args(
    *,
    ...
    ds = require_dataset(
        dataset, purpose="interact with a registry", check_installed=True
    )
    repo = ds.repo
    ds_id = ds.id
    if not ds_id:
        raise ValueError(
            "Dataset does not have datalad.dataset.id set: {}".format(ds.path)
        )
    ...
    endpoint = endpoint or repo.config.get(
        "datalad_registry.endpoint", DEFAULT_ENDPOINT
    )
    return dict(ds=ds, ds_id=ds_id, sibling=sibling, urls=urls, endpoint=endpoint)

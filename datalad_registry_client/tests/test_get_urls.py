...
class TestRegistryGetURLs:
    ...
    @pytest.mark.parametrize(
        "ending_status_code, msg_content",
        [
    ...
    def test_handle_error_response(self, ending_status_code, msg_content, monkeypatch):
        """
        Test handling of an error response from the server

        :param ending_status_code: The HTTP status code of the ending response
                                   from the server
        ...
        # noinspection PyUnusedLocal
        def mock_get(s, url):  # noqa: U100 Unused argument
            return MockResponse(ending_status_code, "")
